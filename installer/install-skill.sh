...
[[ -f "$ROOT/install-state.conf" ]] || { echo "ERROR: free AI Builder OS framework is required. Install it first."; exit 1; }

eligible=()
while IFS='|' read -r id rel version; do
  [[ -f "$HOME/$rel/ai-builder-os/SKILL.md" ]] && eligible+=("$id|$rel")
