...
$stateFile=Join-Path $Root 'install-state.conf'; if(-not(Test-Path $stateFile)){Write-Output 'ERROR: free AI Coding OS framework is required.'; exit 1}
$installed=0
foreach($line in Get-Content $stateFile){$p=$line -split '\|'; $core=Join-Path (Join-Path $HOME $p[1]) 'ai-coding-os\SKILL.md'; if(Test-Path $core){$a=Read-Host "Install '$name' for $($p[0])? [Y/n]"; if(-not $a -or $a -match '^[Yy]$'){$dest=Join-Path (Join-Path $HOME $p[1]) $name; if(Test-Path $dest){Remove-Item $dest -Recurse -Force}; Copy-Item $pack $dest -Recurse; $installed++}}}
